...
static String mockConfirmationId = "";
 ...
 *
 * For any valid UUID v4 confirmation ID generated by BootId, writing to NVS
 * and then reading back should produce an exact match of the original value.
 */
void test_property4_nvs_persistence_round_trip(void) {
    const int ITERATIONS = 100;
    int successCount = 0;

    for (int i = 0; i < ITERATIONS; i++) {
        // Generate a valid UUID v4 confirmation ID using BootId
        String originalConfirmationId = BootId::generate();

        // Verify it's a valid UUID v4
        TEST_ASSERT_TRUE_MESSAGE(BootId::isValidUuid(originalConfirmationId),
                                 "Generated UUID should be valid");

        // Simulate write to NVS
        mockConfirmationId = originalConfirmationId;

        // Simulate read from NVS
        String retrievedConfirmationId = mockConfirmationId;

        // Verify exact match
        if (originalConfirmationId == retrievedConfirmationId) {
            successCount++;
        } else {
            char msg[512];
            snprintf(msg, sizeof(msg),
                     "Round-trip failed at iteration %d:\n"
                     "  Original: %s\n"
                     "  Retrieved: %s",
                     i, originalConfirmationId.c_str(), retrievedConfirmationId.c_str());
            TEST_FAIL_MESSAGE(msg);
            return;
        }
    }
...
int main(int argc, char** argv) {
    ...
    // Property 4: NVS persistence round-trip
    RUN_TEST(test_property4_nvs_persistence_round_trip);
